...
/** Indian digit grouping, no symbol: 4800000 -> "48,00,000" */
function inr_group($n): string
{
    $n = (float) $n;
    $neg = $n < 0 ? '-' : '';
...
/** Trim trailing zeros: 4.80 -> "4.8", 5.00 -> "5" */
function num_trim(float $v): string
{
    return rtrim(rtrim(number_format($v, 1, '.', ''), '0'), '.');
}

/** Full money format: ₹12,34,567 (sign before symbol: -₹32,800) */
function money($n): string
{
    $n = (float) $n;
    return ($n < 0 ? '-' : '') . APP_CURRENCY . inr_group(abs($n));
}

/** Compact money for stat tiles (Indian): ₹4.8Cr / ₹3.2L / ₹45K */
function money_short($n): string
{
    $n = (float) $n;
    $neg = $n < 0 ? '-' : '';
    $a = abs($n);
    if ($a >= 10000000) return $neg . APP_CURRENCY . num_trim($a / 10000000) . 'Cr';
    if ($a >= 100000)   return $neg . APP_CURRENCY . num_trim($a / 100000) . 'L';
    if ($a >= 1000)     return $neg . APP_CURRENCY . number_format($a / 1000, 0) . 'K';
    return $neg . APP_CURRENCY . number_format($a, 0);
}
...
{
    $u = current_user();
    return $u && strtolower($u['email'] ?? '') === strtolower(ADMIN_EMAIL);
}
...
/** ---- Form field helpers (return HTML) ------------------------ */

function field_input(string $label, string $name, $val = '', string $type = 'text', string $ph = '', string $attr = ''): string
{
...
function field_select(string $label, string $name, array $options, $selected = ''): string
{
    $h = '<div><label class="block text-xs font-semibold text-slate-600 mb-1.5">' . e($label) . '</label>'
    ...
    foreach ($options as $v => $lbl) {
        $h .= '<option value="' . e($v) . '"' . ((string) $v === (string) $selected ? ' selected' : '') . '>' . e($lbl) . '</option>';
    }
    return $h . '</select></div>';
}

function field_textarea(string $label, string $name, $val = '', int $rows = 3): string
{
...
/**
 * Inline SVG icon (lucide-style). Usage: icon('users', 18, 'text-blue-600')
 */
function icon(string $name, int $size = 18, string $class = ''): string
{
    $paths = [
        ...
        'sticky-note'   => '<path d="M15.5 3H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h8.5L21 13.5V5a2 2 0 0 0-2-2Z"/><path d="M15 3v6h6"/>',
    ...
    return '<svg xmlns="http://www.w3.org/2000/svg" width="' . $size . '" height="' . $size
        ...
        . 'stroke-linecap="round" stroke-linejoin="round" class="' . e($class) . '">' . $p . '</svg>';
}
